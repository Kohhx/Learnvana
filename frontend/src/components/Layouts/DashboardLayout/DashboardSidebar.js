    ...
    if (!config.end) {
      return (
        <div>
          {!config.button ? (
            <NavLink
              to={config.link}
              className="flex gap-5 items-center text-2xl hover:scale-105 transition-all mb-7"
            >
              {config.icon && config.icon}
              <h4 className="text-lg text-proj-grey3-300">{config.name}</h4>
            </NavLink>
          ) : (
            <button
              className="flex gap-5 items-center text-2xl hover:scale-105 transition-all mb-7"
      ...
      return (
        <div className="grow flex">
          <NavLink
            to={config.link}
            className="flex gap-5 items-center text-2xl hover:scale-105 transition-all mb-7 self-end"
          >
            {config.icon && config.icon}
            <h4 className="text-lg text-proj-grey3-300">{config.name}</h4>
          </NavLink>
      ...
      <div className="w-full min-h-full mx-auto flex flex-col px-3 overflow-y-auto">
        <Link to="/">
          <div className="mx-auto flex items-center gap-4 mb-12">
            ...
            <h1 className="text-[1.6rem] font-kaisei font-normal">Learnvana</h1>
          </div>
        </Link>
        {sideBarLinks}
      </div>
