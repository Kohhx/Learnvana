...
exports.getInstructorClass = asyncHandler(async (req, res) => {
  ...
  if (!instructor) {
    res.status(400);
    throw new Error("No instructor profile created. Create instructor profile first");
  }
...
exports.addPendingUserToClass = asyncHandler(async (req, res) => {
  ...
  const existingStudent = studentsInClass.find(studentInClass => studentInClass.toString() === studentId);
  if (existingStudent) {
    res.status(400);
...
exports.approveStudentFromClass = asyncHandler(async (req, res, next) => {
  ...
  const finddStudent = classFound.pending.find(pendingStudent => pendingStudent._id.toString() == student._id.toString());
  if (!finddStudent) {
    res.status(500);
...
exports.deleteStudentFromClass = asyncHandler(async (req, res, next) => {
  ...
  const finddStudent = classFound.students.find(classStudent => classStudent._id.toString() == student._id.toString());
  if (!finddStudent) {
    res.status(500);
